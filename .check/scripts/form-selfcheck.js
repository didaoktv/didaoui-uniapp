...
const rules_1 = require("../components/dd-form/rules");
let failed = 0;
...
async function assertErr(value, rules, trigger, expected, msg) {
    const got = await (0, rules_1.runRules)(value, rules, trigger);
    assert(got === expected, `${msg}: expected ${JSON.stringify(expected)}, got ${JSON.stringify(got)}`);
}
async function main() {
    // === isEmptyValue 边界 ===
    assert((0, rules_1.isEmptyValue)('') === true, 'isEmptyValue("") 应为空');
    assert((0, rules_1.isEmptyValue)(null) === true, 'isEmptyValue(null) 应为空');
    assert((0, rules_1.isEmptyValue)(undefined) === true, 'isEmptyValue(undefined) 应为空');
    assert((0, rules_1.isEmptyValue)([]) === true, 'isEmptyValue([]) 应为空');
    assert((0, rules_1.isEmptyValue)('a') === false, 'isEmptyValue("a") 应非空');
    assert((0, rules_1.isEmptyValue)(0) === false, 'isEmptyValue(0) 应非空');
    assert((0, rules_1.isEmptyValue)(false) === false, 'isEmptyValue(false) 应非空');
    assert((0, rules_1.isEmptyValue)([1]) === false, 'isEmptyValue([1]) 应非空');
    // === required ===
    ...
    await assertErr('dup', [{ validator: () => Promise.resolve(false), message: '重复' }], undefined, '重复', 'validator 异步 false');
    await assertErr('dup', [{ validator: () => Promise.resolve('异步错误') }], undefined, '异步错误', 'validator 异步字符串');
    // === trigger 过滤 ===
    const onBlurOnly = [{ required: true, message: '必填', trigger: 'onBlur' }];
    ...
    ], undefined, '第一', '多规则首个失败短路');
    await assertErr('b', [
        { required: true, message: '第一' },
        { pattern: /a/, message: '第二' },
    ], undefined, '第二', '多规则顺序执行到第二个');
    // === 空规则集 ===
